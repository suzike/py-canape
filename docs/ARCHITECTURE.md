...
- `diagnostics.py`：UDS 清单、NRC 语义、会话/安全状态机、Tester Present 和 DTC 快照；
...
| 诊断状态或 NRC 被误判 | 清单前置门禁、原始响应留证、NRC 分类和成功后显式状态迁移 |
| ResponsePending 导致重复危险请求 | 进入 P2* 等待语义但不自动重发非幂等请求 |
